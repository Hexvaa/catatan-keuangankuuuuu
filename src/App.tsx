...
interface Transaction {
  id: string; type: TransType; category: string; label: string; amount: number; date: string; photo?: string
}
...
function prepareForOcr(dataUrl: string): Promise<string[]> {
  return new Promise(resolve => {
    const img = new Image()
    img.onload = () => {
      const results: string[] = []
      // Crop configs: [yStart ratio, yEnd ratio]
      const crops: [number, number][] = [[0.45, 1.0], [0.0, 1.0]]
      for (const [yStart, yEnd] of crops) {
        const srcX = 0
        const srcY = Math.round(img.height * yStart)
        const srcW = img.width
        const srcH = Math.round(img.height * (yEnd - yStart))
        // Scale so shortest side ≥ 800px for Tesseract accuracy
        const scale = Math.max(1, 800 / Math.min(srcW, srcH))
        ...
        // Grayscale + contrast stretch (no hard threshold to preserve thin strokes)
        const id = ctx.getImageData(0, 0, dstW, dstH)
        const d = id.data
        for (let i = 0; i < d.length; i += 4) {
          const gray = 0.299*d[i] + 0.587*d[i+1] + 0.114*d[i+2]
          const c = Math.min(255, Math.max(0, (gray - 80) * 1.6))
          d[i] = d[i+1] = d[i+2] = c
        }
...
function AddModal({ onSave, onClose }: { onSave:(t:Transaction)=>void; onClose:()=>void }) {
  ...
  const runOcr = async (dataUrl: string) => {
    ...
    try {
      ...
      const found = await ocrWithApi(dataUrl) ?? await ocrWithTesseract(dataUrl)
  ...
  const ocrWithTesseract = async (dataUrl: string): Promise<number> => {
    const crops = await prepareForOcr(dataUrl)
  ...
  const save  = () => {
    setTried(true)
    if(!valid) return
    onSave({id:Date.now().toString(), type, category:cat||"Lainnya", label:label||(cat||"Lainnya"), amount:parseInt(rawAmount), date, photo: photo || undefined})
  }
...
const [settingsTab, setSettingsTab]   = useState<"nama"|"username"|"password">("nama")
  ...
  return (
    <div style={{ minHeight:"100vh", background:P.bg, transition:"background 0.4s", paddingBottom:100 }}>
        ...
        <div className="au" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          ...
          <button onClick={()=>setAdd(true)}
            style={{ padding:"10px 18px", fontSize:13, fontWeight:600, border:"none", cursor:"pointer",
              ...
              background:isLight?"linear-gradient(135deg,#16a34a,#22c55e)":C.grad,
        ...
        {([
          ...
          return (
            <button key={tab.id} onClick={()=>{
              ...
              {/* Active blob glow */}
              {isActive && (
                <div className="nav-blob" style={{
                  position:"absolute", top:-2, left:"50%", transform:"translateX(-50%)",
                  width:44, height:44, borderRadius:"50%",
                  background: `radial-gradient(circle, ${C.accent}40 0%, ${C.accent}00 70%)`,
              ...
              {isActive && (
                <div className="nav-blob" style={{
                  ...
                  width:52, height:36, borderRadius:18,
                  background: isLight
                    ? `${C.accent}14`
                    : `${C.accent}22`,
                  pointerEvents:"none",
              ...
              {/* Icon */}
              <div className={isActive ? "nav-icon-active" : ""} style={{
                color: isActive ? C.accent : C.muted,
                position:"relative", zIndex:1,
                filter: isActive ? `drop-shadow(0 0 6px ${C.accent}88)` : "none",
                transition: isActive ? "none" : "color 0.2s, filter 0.2s",
              }}>
