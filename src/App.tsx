      ...
      <div style={{ maxWidth:520, margin:"0 auto", padding:"48px 20px 80px", position:"relative" }}>
        ...
        <div className="au d2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
          {([
            ...
            <div key={s.label} className={`grad-border${isAuto?" auto-border":""}`}
              ...
              <p style={{ fontFamily:"DM Mono, monospace", fontSize:14, color:isPlain?C.text:s.color, lineHeight:1, fontWeight:500,
        ...
        <div className={`au d3 grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, padding:"20px 20px", marginBottom:24 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              {label:"Pemasukan",  value:Math.max(0,...monthTxns.filter(t=>t.type==="income" ).map(t=>t.amount)), color:C.green, bg:C.greenDim},
              {label:"Pengeluaran", value:Math.max(0,...monthTxns.filter(t=>t.type==="expense").map(t=>t.amount)), color:C.red,   bg:C.redDim},
            ].map(s=>(
              <div key={s.label} style={{ background:isPlain?C.surface:s.bg, borderRadius:12, padding:"12px 14px", border:`1px solid ${C.border}` }}>
                <p style={{ fontSize:10, color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6, fontWeight:600 }}>{s.label}</p>
                <p style={{ fontFamily:"DM Mono, monospace", fontSize:14, color:isPlain?C.text:s.color, fontWeight:500,
                  letterSpacing: showAmounts ? "normal" : "0.12em" }}>
        ...
        <div className="au d4">
          ...
          <div className={`grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, overflow:"hidden" }}>
            {visible.length===0 ? (
