...
export default function App() {
  ...
  const addTxn = async (t: Transaction) => {
    if (!user) return
    const { id, type, category, label, amount, date, photo } = t
    const { error } = await supabase.from("transactions").insert({ id, type, category, label, amount, date, photo, user_id: user.id })
