    ...
    <div style={{ display:"flex", alignItems:"center", gap:24 }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", gap:12 }}>
        {[
          ...
          { label:"Saldo",       value:saldo,   color: saldo>=0 ? "#6366f1" : C.red },
        ].map(s=>(
...
// ─── Month Swipe Navigator ────────────────────────────────────────────────────

function MonthSwipeNav({ month, year, setMonth, monthHasData, P }: {
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(dx) > 40 && dy < 60) {
      if (dx < 0) setMonth(m => m === 11 ? 0 : m + 1)
      else        setMonth(m => m === 0  ? 11 : m - 1)
    }
    swipeRef.current = null
  }

  return (
    <div className="au" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
      style={{ marginBottom:20, userSelect:"none" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
        <button onClick={()=>setMonth(m=>m===0?11:m-1)}
          style={{ width:32, height:32, borderRadius:9, border:`1px solid ${P.border}`, background:P.surface,
            cursor:"pointer", color:P.text, display:"flex", alignItems:"center", justifyContent:"center",
        ...
        {Array.from({length:12},(_,i)=>(
          <button key={i} onClick={()=>setMonth(i)}
            style={{ padding:0, border:"none", cursor:"pointer",
              width: i===month ? 20 : 6, height:6, borderRadius:3,
              background: i===month ? P.grad : monthHasData[i] ? P.accent+"55" : P.border,
              transition:"all 0.25s cubic-bezier(0.22,1,0.36,1)",
              boxShadow: i===month ? `0 0 8px ${P.accent}66` : "none",
            }}/>
        ))}
      </div>
    </div>
...
function Card3D({ children, style, className, ...rest }: { children: React.ReactNode; style?: React.CSSProperties; className?: string; [key: string]: unknown }) {
  ...
  const onMove = useCallback((e: React.MouseEvent) => {
    ...
    el.style.transform = `perspective(700px) rotateY(${x * 14}deg) rotateX(${-y * 10}deg) translateZ(6px) scale(1.015)`
    el.style.setProperty("--mx", `${(x + 0.5) * 100}%`)
    el.style.setProperty("--my", `${(y + 0.5) * 100}%`)
    const shadow = el.dataset.shadow
    if (shadow) el.style.boxShadow = shadow.replace("base", `${-x*18}px ${-y*18}px 40px`)
  }, [])
  ...
  return (
    <div ref={ref} className={`card-3d${className ? " " + className : ""}`}
      style={{ position:"relative", ...style }}
      onMouseMove={onMove} onMouseLeave={onLeave} {...rest as React.HTMLAttributes<HTMLDivElement>}>
...
const cats  = type==="income" ? INCOME_CATS : EXPENSE_CATS
  ...
  const save  = () => {
    setTried(true)
    if(!valid) return
    onSave({id:crypto.randomUUID(), type, category:cat||"Lainnya", label:label||(cat||"Lainnya"), amount:parseInt(rawAmount), date, time: new Date().toLocaleTimeString("id-ID", {hour:"2-digit",minute:"2-digit",timeZone:"Asia/Jakarta"}), photo: photo || undefined})
  }
        ...
        <div style={{ padding:"14px 22px", borderTop:`1px solid ${C.border}`, display:"flex", gap:8, flexShrink:0 }}>
          ...
          <button onClick={save}
            style={{ flex:2, padding:11, borderRadius:10, border:"none", fontSize:13, fontWeight:600,
              ...
              opacity: ocrStatus==="loading" ? 0.65 : 1,
...
function LiveClock({ textColor="inherit", mutedColor="rgba(128,128,128,0.7)", small=false }: { textColor?:string; mutedColor?:string; small?:boolean }) {
  const [t, setT] = useState(new Date())
  useEffect(() => { const id = setInterval(() => setT(new Date()), 1000); return () => clearInterval(id) }, [])
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent: small ? "center" : "flex-start", gap:6 }}>
      <span style={{ fontSize: small ? 11 : 18, fontWeight: small ? 500 : 700, fontFamily:"DM Mono, monospace", letterSpacing:"0.04em", color: small ? mutedColor : textColor }}>
        ...
        <div className="au" style={{ animationDelay:"0.07s", marginBottom:18 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, justifyContent:"center" }}>
            ...
            <div style={{ position:"relative", width:52, height:60, flexShrink:0 }}>
              ...
              {/* Wallet */}
              <svg className="wallet-anim" width="56" height="48" viewBox="0 0 80 64" fill="none" style={{ position:"absolute", bottom:0 }}>
                <defs>
                  ...
                  <linearGradient id="wflap" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="100%" stopColor="#B07035"/>
                  </linearGradient>
                  <linearGradient id="wpocket" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#6B3E1E"/>
                    <stop offset="100%" stopColor="#4A2810"/>
                  </linearGradient>
                  <linearGradient id="wcoin" x1="0" y1="0" x2="0" y2="1">
                ...
                {/* Body */}
                <rect x="2" y="14" width="76" height="48" rx="12" fill="url(#wbody)" filter="url(#wshadow)"/>
                {/* Top flap */}
                <rect x="2" y="14" width="76" height="20" rx="12" fill="url(#wflap)"/>
                <rect x="2" y="26" width="76" height="8" fill="url(#wflap)"/>
                {/* Stitching line */}
                <path d="M12 18 L68 18" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" strokeDasharray="4 3" strokeLinecap="round"/>
                {/* Card lines */}
                <rect x="10" y="20" width="28" height="3.5" rx="1.8" fill="rgba(255,255,255,0.18)"/>
                <rect x="10" y="26" width="18" height="3" rx="1.5" fill="rgba(255,255,255,0.12)"/>
                {/* Pocket */}
                <rect x="50" y="30" width="26" height="22" rx="8" fill="url(#wpocket)"/>
                <rect x="51" y="31" width="24" height="8" rx="4" fill="rgba(255,255,255,0.06)"/>
                {/* Coin */}
                <circle className="coin-anim" cx="63" cy="41" r="7" fill="url(#wcoin)" style={{ transformOrigin:"63px 41px" }}/>
                <circle cx="63" cy="41" r="4.5" fill="#22c55e"/>
                <text x="60" y="45" fontSize="5.5" fill="#fff" fontWeight="bold" fontFamily="sans-serif">Rp</text>
                {/* Shine */}
                <ellipse cx="20" cy="18" rx="14" ry="3" fill="rgba(255,255,255,0.12)"/>
              </svg>
            </div>
          </div>
          <p style={{ fontFamily:"Fraunces, serif", fontSize:"clamp(40px,9vw,68px)", fontWeight:700, fontStyle:"italic",
...
export default function App() {
  ...
  const submitTopup = () => {
    ...
    setTopupTarget(null); setTopupAmount(""); setTopupNote(""); setTopupDate(localToday())
  }
  ...
  const periodFiltered = useMemo(() => {
    ...
    return source.filter(t => {
      const d = new Date(t.date + "T00:00:00")
      if (period === "today")    return t.date === localToday()
  ...
  const visible = (filter==="all" ? periodFiltered : periodFiltered.filter(t=>t.type===filter))
    .filter(t => {
      ...
      return t.label.toLowerCase().includes(q) ||
        ...
        t.amount.toString().includes(q.replace(/\./g,""))
    })
  ...
  const addTxn = async (t: Transaction) => {
    if (!user) return
    const { id, type, category, label, amount, date } = t
    const { error } = await supabase.from("transactions").insert({ id, type, category, label, amount, date, user_id: user.id })
    if (error) { console.error("[addTxn]", error); alert("Gagal simpan: " + error.message); return }
...
const [settingsTab, setSettingsTab]   = useState<"nama"|"username"|"password">("nama")
  ...
  return (
    <div style={{ minHeight:"100vh", background:P.bg, paddingBottom:120 }}>
      ...
      <div style={{ maxWidth:520, margin:"0 auto", padding:"48px 20px 80px", position:"relative" }}>
        ...
        <div className="au" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28, position:"relative", zIndex:50 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:32, height:32, borderRadius:"50%", background:P.accentDim,
              display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>{avatar}</div>
        ...
        <div className="au d1" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
          ...
          <div style={{ textAlign:"center" }}>
            <p style={{ fontSize:15, fontWeight:700, color:P.text }}>{MONTHS[month]} {year}</p>
            <LiveClock textColor={P.text} mutedColor={P.muted} small/>
          </div>
          <button onClick={()=>setMonth(m=>m===11?0:m+1)}
            style={{ width:34, height:34, borderRadius:9, border:`1px solid ${P.border}`, background:P.surface, cursor:"pointer", color:P.text, display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.15s", flexShrink:0 }}
            onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background=P.accentDim}
        ...
        <div className="au d2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
          {([
            ...
            { label:"Saldo", value:balance, color:balance>=0?"#6366f1":C.red,
              dim:balance>=0?"rgba(99,102,241,0.1)":C.redDim,
              glow:`0 6px 28px rgba(99,102,241,0.22)`, glowHover:`0 12px 40px rgba(99,102,241,0.42)`,
              gradLine:balance>=0?"linear-gradient(90deg,#6366f1,#818cf8)":"linear-gradient(90deg,#f87171,#fb923c)", icon:"◈" },
          ] as const).map(s=>(
            <Card3D key={s.label} className={`grad-border${isAuto?" auto-border":""}`}
              style={{ background: C.card, borderRadius:16, padding:"18px 13px 14px" }}>
        ...
        {/* Month chart */}
        <Card3D className={`au d3 grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, padding:"20px 20px", marginBottom:24 }}>
        ...
        <div className="au" style={{ animationDelay:"0.3s", marginBottom:24 }}>
          <Card3D className={`grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, padding:"18px 18px 14px" }} data-cal>
        ...
        <div className="au" style={{ animationDelay:"0.31s", marginBottom:24 }}>
          <Card3D className={`grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, padding:"18px 18px 14px" }}>
        ...
        {/* Txn page header */}
        <div className="au" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
          <div>
        ...
        {/* Month swipe navigator */}
        <MonthSwipeNav month={month} year={year} setMonth={setMonth} monthHasData={monthHasData} P={C}/>

        {/* Search bar */}
        <div style={{ position:"relative", marginBottom:14 }}>
          <svg style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}
            width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="Cari nama, kategori, atau nominal..."
            style={{ width:"100%", padding:"10px 36px 10px 36px", fontSize:13, border:`1px solid ${C.border}`,
          ...
          return (
            <div className="au" style={{ background:C.card, borderRadius:16, padding:"20px 18px", marginTop:16, border:`1px solid ${C.border}` }}>
              ...
              <div style={{ display:"flex", gap:12, marginBottom:12 }}>
                {[{color:C.green, label:"Pemasukan"},{color:C.red, label:"Pengeluaran"}].map(l=>(
                  <span key={l.label} style={{ display:"flex", alignItems:"center", gap:5, fontSize:10, color:C.muted, fontWeight:600 }}>
                    <span style={{ width:8, height:8, borderRadius:2, background:l.color, display:"inline-block" }}/>{l.label}
                  </span>
                ))}
              </div>
              {/* Grafik gabungan */}
              <div style={{ display:"flex", gap:2, alignItems:"flex-end", height:barH, marginBottom:16 }}>
                {monthLabels.map((m,i) => (
                  <div key={m} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
                    <div style={{ width:"100%", display:"flex", gap:"1px", alignItems:"flex-end", height:barH-14 }}>
                      <div style={{ flex:1, background:C.green, borderRadius:"3px 3px 0 0", height:`${(thisData[i].inc/maxVal)*100}%`, minHeight: thisData[i].inc>0?2:0, transition:"height 0.6s cubic-bezier(0.22,1,0.36,1)" }}/>
                      <div style={{ flex:1, background:C.red,   borderRadius:"3px 3px 0 0", height:`${(thisData[i].exp/maxVal)*100}%`, minHeight: thisData[i].exp>0?2:0, transition:"height 0.6s cubic-bezier(0.22,1,0.36,1)" }}/>
                    </div>
          ...
          <div className="au">
            ...
            <div className={`grad-border${isAuto?" auto-border":""}`}
              ...
              <div style={{ position:"relative", flexShrink:0 }}>
                <div style={{ width:56, height:56, borderRadius:"50%", background:C.accentDim,
                  display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>{avatar}</div>
                <button onClick={()=>setShowAvatarPicker(true)}
                  style={{ position:"absolute", bottom:0, right:0, width:20, height:20, borderRadius:"50%",
                    background:C.accent, border:`2px solid ${C.card}`, cursor:"pointer",
                    display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:10, color:"#fff", padding:0 }}>✏️</button>
              </div>
              <div>
      ...
      {showAvatarPicker && (
        <div style={{ position:"fixed", inset:0, zIndex:1000, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
          ...
          <div style={{ background:P.card, borderRadius:20, padding:"24px 20px", width:"100%", maxWidth:340,
            boxShadow:"0 24px 60px rgba(0,0,0,0.4)" }} onClick={e=>e.stopPropagation()}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
              <p style={{ fontFamily:"Fraunces, serif", fontSize:18, fontWeight:700, color:P.text }}>Pilih Avatar</p>
              <button onClick={()=>setShowAvatarPicker(false)}
                style={{ background:"none", border:"none", cursor:"pointer", color:P.muted, fontSize:18, padding:4 }}>✕</button>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:8 }}>
              {AVATARS.map(a => (
                <button key={a} onClick={()=>{ setAvatar(a); localStorage.setItem(`avatar_${user?.id}`,a); setShowAvatarPicker(false) }}
                  style={{ width:"100%", aspectRatio:"1", fontSize:24, borderRadius:12, border:`2px solid ${a===avatar?P.accent:"transparent"}`,
                    background: a===avatar ? P.accentDim : P.surface,
              ...
              {settingsTab==="password" && (
                <>
                  <div>
                    ...
                    <div style={{ position:"relative" }}>
                      <input type={showNewPw?"text":"password"} value={settingsNewPw} onChange={e=>setSettingsNewPw(e.target.value)}
                        placeholder="Minimal 6 karakter"
                        style={{ width:"100%", padding:"11px 44px 11px 14px", fontSize:14, border:`1px solid ${P.border}`,
                          borderRadius:10, background:P.surface, color:P.text, outline:"none",
                          fontFamily:"Instrument Sans, sans-serif" }}/>
                      <button type="button" onClick={()=>setShowNewPw(v=>!v)}
                        style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)",
                          background:"none", border:"none", cursor:"pointer", color:P.muted, display:"flex", alignItems:"center", padding:4 }}>
                        {showNewPw
                  ...
                  <div>
                    ...
                    <div style={{ position:"relative" }}>
                      <input type={showConfPw?"text":"password"} value={settingsConfirmPw} onChange={e=>setSettingsConfirmPw(e.target.value)}
      ...
      <div style={{ position:"fixed", bottom:"max(24px, env(safe-area-inset-bottom, 24px))", left:"50%", transform:"translateX(-50%)",
        ...
        {([
          ...
          return (
            <button key={tab.id}
              onTouchEnd={e=>{ e.preventDefault(); setActiveTab(tab.id); if(tab.id==="txn"){ setTimeout(()=>txnRef.current?.scrollIntoView({ behavior:"smooth", block:"start" }),50) } }}
              onClick={()=>{ setActiveTab(tab.id); if(tab.id==="txn"){ setTimeout(()=>txnRef.current?.scrollIntoView({ behavior:"smooth", block:"start" }),50) } }}
