...
export default function App() {
  ...
  const [txns, setTxns]     = useState<Transaction[]>(() => {
    try {
      const saved = localStorage.getItem("ckku_txns")
  ...
  useEffect(() => {
    localStorage.setItem("ckku_txns", JSON.stringify(txns))
  }, [txns])
  ...
  const addTxn   = (t: Transaction) => { setTxns(p => [t, ...p]); setAdd(false); addToast(t.type, t.amount) }
  const saveTxn  = (t: Transaction) => { setTxns(p => p.map(x => x.id === t.id ? t : x)); setEditTxn(null) }
  const deleteTxn = (id: string)    => { setTxns(p => p.filter(x => x.id !== id)); setEditTxn(null) }
  ...
  return (
    <div style={{ minHeight:"100vh", background:P.bg, transition:"background 0.4s" }}>
      ...
      <div style={{ maxWidth:520, margin:"0 auto", padding:"48px 20px 80px", position:"relative" }}>
        ...
        {/* Theme switcher */}
        <div className="au" style={{ display:"flex", justifyContent:"flex-end", marginBottom:28 }}>
