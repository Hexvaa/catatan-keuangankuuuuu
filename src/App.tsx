...
function LandingPage({ onStart }: { onStart: () => void }) {
  const features = [
    { icon:"📊", title:"Ringkasan Visual", desc:"Grafik donut bulanan yang jelas" },
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#faf7f2 0%,#f0ebe3 40%,#e8e0d5 100%)", fontFamily:"Instrument Sans, sans-serif", overflowX:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center",
        ...
        <div className="au" style={{ animationDelay:"0.07s", marginBottom:18 }}>
          <p style={{ fontSize:10, color:"rgba(0,0,0,0.3)", letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:14 }}>Keuangan Pribadi</p>
          <p style={{ fontFamily:"Fraunces, serif", fontSize:"clamp(40px,9vw,68px)", fontWeight:700, fontStyle:"italic",
            color:"#1a1a1a", lineHeight:1.05, letterSpacing:"-1px", marginBottom:2 }}>Catatan</p>
          <p style={{ fontFamily:"Fraunces, serif", fontSize:"clamp(40px,9vw,68px)", fontWeight:700, fontStyle:"italic",
        ...
        <div className="au" style={{ animationDelay:"0.25s", display:"flex", gap:0, marginTop:52, borderRadius:16,
          ...
          {[
            ...
            { value:"Aman", label:"Data Kamu" },
            { value:"Multi", label:"Pengguna" },
          ].map((s,i)=>(
            <div key={s.label} style={{ textAlign:"center", padding:"18px 28px",
              borderRight: i<2 ? "1px solid rgba(0,0,0,0.07)" : "none" }}>
              <p style={{ fontFamily:"Fraunces, serif", fontSize:20, fontWeight:700, color:"#1a1a1a", marginBottom:4 }}>{s.value}</p>
              <p style={{ fontSize:10, color:"rgba(0,0,0,0.35)", fontWeight:600, letterSpacing:"0.07em", textTransform:"uppercase" }}>{s.label}</p>
            </div>
          ))}
        </div>

        {/* Features */}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:44, width:"100%", maxWidth:400 }}>
          {features.map((f,i)=>(
            <div key={f.title} className="au"
              style={{ animationDelay:`${0.3+i*0.05}s`,
                background:"rgba(255,255,255,0.55)", border:"1px solid rgba(0,0,0,0.07)",
                borderRadius:14, padding:"16px 14px", textAlign:"left", backdropFilter:"blur(8px)" }}>
              <span style={{ fontSize:20, display:"block", marginBottom:8 }}>{f.icon}</span>
              <p style={{ fontFamily:"Fraunces, serif", fontSize:13, fontWeight:700, fontStyle:"italic", color:"#1a1a1a", marginBottom:4 }}>{f.title}</p>
              <p style={{ fontFamily:"Fraunces, serif", fontSize:11, color:"rgba(0,0,0,0.45)", lineHeight:1.7 }}>{f.desc}</p>
            </div>
          ))}
        </div>

        <p className="au" style={{ animationDelay:"0.52s", marginTop:44, fontSize:14, color:"rgba(0,0,0,0.35)", fontFamily:"Fraunces, serif", fontStyle:"italic" }}>
          Sudah punya akun?{" "}
    ...
    <div style={{ minHeight:"100vh", background:P.bg, transition:"background 0.4s" }}>
      ...
      <div style={{ maxWidth:520, margin:"0 auto", padding:"48px 20px 80px", position:"relative" }}>
        ...
        <div className="au d4">
          ...
          {showTxnList && <div className={`grad-border${isAuto?" auto-border":""}`} style={{ background:C.card, borderRadius:16, overflow:"hidden" }}>
            {visible.length===0 ? (
                ...
                return (
                  <div key={g.date}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                      padding:"10px 18px 8px", background:`${C.surface}88`,
                      borderBottom:`1px solid ${C.border}` }}>
                      <p style={{ fontSize:12, fontWeight:700, color:C.text }}>
                        {d.getDate()} {MONTHS_S[d.getMonth()]} {d.getFullYear()}
                      </p>
                      <div style={{ display:"flex", gap:10 }}>
                        {dayInc>0 && <span style={{ fontFamily:"DM Mono, monospace", fontSize:11, fontWeight:600, color:isLight?C.text:C.green }}>
                          {showAmounts ? `+${rp(dayInc)}` : "••••"}
                        </span>}
                        {dayExp>0 && <span style={{ fontFamily:"DM Mono, monospace", fontSize:11, fontWeight:600, color:isLight?C.text:C.red }}>
